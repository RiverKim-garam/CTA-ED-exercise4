...
```{r}
#### estimate the Wordfish model ####
set.seed(123L) #Is other seed setting available? 
...
```
Features are not described properly, I guess, because they are written in Japanese. 
So I need to look features and figure out what they mean.

Look features
```{r}
features_kaneko <- kaneko_dfm_results[["features"]]

betas_kaneko <- kaneko_dfm_results[["beta"]]

feat_betas_kaneko <- as.data.frame(cbind(features_kaneko, betas_kaneko))
feat_betas_kaneko$betas_kaneko <- as.numeric(feat_betas_kaneko$betas_kaneko)

feat_betas_kaneko %>%
  arrange(desc(betas_kaneko)) %>%
  top_n(20) %>% 
  kbl() %>%
  kable_styling(bootstrap_options = "striped")
```
So now we can check which features are evaluated with high betas.